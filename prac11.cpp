...
void DatabaseFile :: AddNewRecord()
{
    ...
    if(flag_interating_through_file == 1)  //if roll no. is unique then only write to the file
    {
        ...
        string date;
        getline(cin, date);
        stud_rec.stud_DoB.day = stoi(date.substr(0, 2));//stoi() function to convert string to integer
        stud_rec.stud_DoB.month = stoi(date.substr(3, 2));//substr() to extract data from string( first column 
        stud_rec.stud_DoB.year = stoi(date.substr(6, 4));//starting index, 2nd colmn size to extract)
